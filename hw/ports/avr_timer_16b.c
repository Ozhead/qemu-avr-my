...
#define MODE_PHASE_PWM_8    1
#define MODE_PHASE_PWM_9    2
#define MODE_PHASE_PWM_10   3
#define MODE_CTC_OCRA       4
#define MODE_FAST_PWM_8     5
#define MODE_FAST_PWM_9     6
#define MODE_FAST_PWM_10    7
#define MODE_PHASE_FREQUENCY_PWM_ICR    8
#define MODE_PHASE_FREQUENCY_PWM_OCRA   9
#define MODE_PHASE_PWM_ICRA 10
#define MODE_PHASE_PWM_OCRA 11
#define MODE_CTC_ICR        12
#define MODE_FAST_PWM_ICRA  14
#define MODE_FAST_PWM_OCRA  15
...
static int avr_timer_16b_is_active(void *opaque, uint32_t pinno)
{
    ...
    // no PWM at all
    if(MODE(t16) == MODE_NORMAL || MODE(t16) == MODE_CTC_OCRA || MODE(t16) == MODE_CTC_ICR)
    {
        return 0;
    }
...
static void avr_timer_16b_set_alarm(AVRPeripheralState *t16)
{
    ...
    switch (MODE(t16)) 
    {
        ...
        break;
        case MODE_FAST_PWM_8:
        case MODE_FAST_PWM_9:
        case MODE_FAST_PWM_10:
        case MODE_FAST_PWM_ICRA:
        case MODE_FAST_PWM_OCRA:
            break;
        case MODE_CTC_ICR:
        // CTC mode, top = icr 
        if (ICR(t16) < alarm_offset && ICR(t16) > CNT(t16)) {
...
static uint64_t avr_timer_16b_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    switch(addr)
    {
        ...
        case 4: // OCRB
            return p->ocrb;
        default:
            printf("Error: Wrong Hardware ID in Counter 16b\n");
    }
    return 0;
}
...
static void avr_timer_16b_write(void *opaque, hwaddr addr, uint64_t value,
                                unsigned int size)
{
    ...
    case 3:     // OCRA*/
        ...
        t16->ocra = val8;
        avr_timer_16b_toggle_pwm(t16);   // OCRA can also lead to a change! because it changes the frequency!
        break;
    case 4:     // OCRB
        t16->ocrb = val8;
        avr_timer_16b_toggle_pwm(t16);   
        break;
    default:
        printf("Writing to AVR Timer 16b that is not defined??\n");
        break;
    }
    //printf("Set alarm...\n");
    avr_timer_16b_set_alarm(t16);
...
static void avr_timer_16b_interrupt(void *opaque)
{
    ...
    // Check for ocra overflow in CTC mode 
    if (mode == MODE_CTC_OCRA && t16->next_interrupt == INTERRUPT_COMPA) 
    {
        printf("CTC OCRA overflow\n");
        avr_timer_16b_clock_reset(t16);
    }
        // Check for icr overflow in CTC mode 
    if (mode == MODE_CTC_ICR && t16->next_interrupt == INTERRUPT_CAPT) {
        //DB_PRINT("CTC ICR overflow");
        avr_timer_16b_clock_reset(t16);
        if (t16->imsk & T16_INT_IC) {
            t16->ifr |= T16_INT_IC;
...
static const MemoryRegionOps avr_timer_16b_imsk_ops = {
    .read = avr_timer_16b_read_imsk,
    .write = avr_timer_16b_write_imsk,
    .endianness = DEVICE_NATIVE_ENDIAN,
    .impl = {.max_access_size = 1}
};

static const MemoryRegionOps avr_timer_16b_ifr_ops = {
    .read = avr_timer_16b_read_ifr,
    .write = avr_timer_16b_write_ifr,
    .endianness = DEVICE_NATIVE_ENDIAN,
    .impl = {.max_access_size = 1}
};
...
static void avr_timer_16b_init(Object *obj)
{
    ...
    memory_region_init_io(&s->mmio, obj, &avr_timer_16b_ops, s, TYPE_AVR_TIMER_16b, 8);
